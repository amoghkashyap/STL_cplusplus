...
public:
	...
	bool operator==(const ManagedObject& rhs) const{
		return m_clusterId + m_id == rhs.m_clusterId + rhs.m_id;
	}
...
int main() {
	...
	ManagedObject toFind;
	toFind.m_clusterId = "MGW-Tiger";
	toFind.m_id = "Shelf-1";

	auto iterFoundLocation = find(mos.begin(),mos.end(),toFind);

	if(iterFoundLocation != mos.end()){
		cout<<"Found the MO "<<*iterFoundLocation << endl;
	}
	else{
		cout<<"Not found "<<endl;
	}
